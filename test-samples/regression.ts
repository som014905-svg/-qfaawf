...
import { analyzeLuastSource, rejectLuastBadRewrites } from "../src/passes/luast-analysis";
...
t("Luast pool analysis removes shadowing nil predeclarations", () => {
  const source = "-- generated by luast v1.0.1 https://luast.clv.cloud\n" +
    "local T = {}; T[2] = nil; T[4] = nil; T[7] = nil; T[8] = nil; T[128] = nil; " +
    "T[128] = {\"name\", 1690, function() return true end}; " +
    "local num1; local num37; num37 = 2034 - num37; num1();";
  const result = analyzeLuastSource(source);
  assert.ok(!result.result.includes("T[128] = nil"));
  assert.ok(result.callableNames.includes("num1"));
  assert.ok(result.dispatcherNames.includes("num37"));
  assert.ok(!result.callableNames.includes("num37"));
});

t("Luast suspicious rewrites are reported without corrupting source", () => {
  const source = "local x = 0.02[3584]\nlocal y = \"name\" * n";
  const result = rejectLuastBadRewrites(source);
  assert.equal(result.result, source);
  assert.ok(result.notes.some((note) => note.includes("suspicious Luast rewrite")));
});
